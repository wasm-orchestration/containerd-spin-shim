...
pub fn prepare_module(bundle: String) -> Result<(PathBuf, PathBuf), Error> {
    info!("==V== The received bundle is {}", bundle);
...
impl Wasi {
    ...
    async fn build_spin_trigger(
        ...
        info!("==V== Application name: {}", app.info.name);
...
fn generate_random_port() -> String {
    ...
    let random_port = rng.gen_range(30000..65000);
    random_port.to_string()
}

fn redis_persist_port(id: &str, port: &str) {
    info!("==V== Trying to persist port...");
    let client = redis::Client::open("redis://:password@127.0.0.1:9898?db=0");
    if client.is_err() {
        info!(
            "==V== Cannot establish a Redis connection!, {}",
            client.err().unwrap()
        );
        return;
    }
    ...
    if con.is_err() {
        info!(
            "==V== Cannot acquire Redis connection, {}",
            con.err().unwrap()
        );
        return;
    }
    let mut con = con.unwrap();
    let result: Result<(), RedisError> = con.set(id, port);
    if result.is_err() {
        info!(
            "==V== Cannot persist the id and port to Redis, {}",
            result.err().unwrap()
        );
        return;
    }
    let expiry_time = Utc::now() + Duration::minutes(15);
    let result: Result<(), RedisError> =
        con.expire_at(id, expiry_time.timestamp().try_into().unwrap());
    if result.is_err() {
        info!(
            "==V== Cannot add expiration time to key, {}",
            result.err().unwrap()
        );
        return;
    }
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ...
                rt.block_on(async {
                    ...
                    let listen_port = generate_random_port();
                    let spin_socket = format!("{}:{}", SPIN_ADDR, listen_port);
                    redis_persist_port(&outer_id, &listen_port);
                    info!("==V== Running with a random port {}", spin_socket);
                    let f = http_trigger.run(spin_http::CliArgs {
                        address: parse_addr(spin_socket.as_str()).unwrap(),
                        tls_cert: None,
                        tls_key: None,
                    });
